...
# OpenAPI
from OpenAPI.commons import BadRequest, Unauthorized, Forbidden, NotFound, UnavailableService, Headers, CommonRequestBody
from OpenAPI.reinstatement_openapi import ReinstatementResponse
# CONSTANTS
from constants.routes import REINSTATEMENT_ROUTE
from constants.identifiers_constants import ULA, reinstatement_id
# services
...
reinstatement = APIBlueprint(
    'Reinstatement',
    __name__,
    url_prefix=REINSTATEMENT_ROUTE,
    abp_tags=[Tag(name="Reinstatement", description="'Reinstatement' procedure")],
    abp_responses={
        "400": BadRequest,
        "401": Unauthorized,
        "403": Forbidden,
...
@reinstatement.post('', responses={"201": ReinstatementResponse},
                    security=[{"jwt": []}])
def reinstatement_controller(header: Headers, body: CommonRequestBody):
    """
    Endpoint to request "Reincorporacion" procedure to salesforce
    """
    ...
    servicio_response = set_servicio_properties(student_data, body, reinstatement.name)
    # fetch record type
    record_type = fetch_record_type(student_data, reinstatement_id)
    ...
    procedure = 'Reincorporación de Estudios'
    ...
    if student_data.school == ULA:
        ...
        detail_code = fetch_detail_code(student_data, reinstatement_id)
