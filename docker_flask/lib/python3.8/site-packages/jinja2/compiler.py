...
class CodeGenerator(NodeVisitor):
    ...
    def pop_assign_tracking(self, frame: Frame) -> None:
        ...
        if len(vars) == 1:
            ...
            if frame.block_frame:
                ...
                return
            self.writeline(f"context.constants[{name!r}] = {ref}")
        else:
            if frame.loop_frame:
                ...
                self.writeline("context.constants.update({")
    ...
    def visit_Import(self, node: nodes.Import, frame: Frame) -> None:
        ...
        if frame.toplevel:
            self.write(f"context.constants[{node.target!r}] = ")
    ...
    def visit_FromImport(self, node: nodes.FromImport, frame: Frame) -> None:
        ...
        if var_names:
            if len(var_names) == 1:
                name = var_names[0]
                self.writeline(f"context.constants[{name!r}] = {frame.symbols.ref(name)}")
            else:
                names_kv = ", ".join(
                    f"{name!r}: {frame.symbols.ref(name)}" for name in var_names
                )
                self.writeline(f"context.constants.update({{{names_kv}}})")
        if discarded_names:
    ...
    def visit_Macro(self, node: nodes.Macro, frame: Frame) -> None:
        ...
        if frame.toplevel:
            ...
            self.writeline(f"context.constants[{node.name!r}] = ")
    ...
    def visit_OverlayScope(self, node: nodes.OverlayScope, frame: Frame) -> None:
        ...
        self.writeline(f"{ctx}.constants = ")
